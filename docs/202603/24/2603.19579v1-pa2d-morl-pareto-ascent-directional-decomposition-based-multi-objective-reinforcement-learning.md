...
tags: ["query:vsl-rl"]
score: 6.0
evidence: 用于复杂决策的多目标强化学习
tldr: 针对多目标强化学习中处理冲突目标及高维空间的挑战，本文提出PA2D-MORL算法。该方法结合帕累托上升方向进行问题分解与策略改进，利用多目标策略梯度确保所有目标共同提升。通过进化框架优化多策略并引入自适应微调，显著提升了帕累托前沿的近似质量与分布均匀性，在机器人控制任务中表现优于现有主流算法。
selection_source: fresh_fetch
motivation: 旨在解决复杂高维状态动作空间下，多目标强化学习难以获得高质量帕累托策略集近似的问题。
method: 提出一种基于帕累托上升方向的分解方案，结合多目标策略梯度与进化框架，并利用自适应微调优化前沿分布。
result: 在多种多目标机器人控制任务中的实验表明，该方法在结果质量和稳定性方面均显著优于当前最先进的算法。
conclusion: PA2D-MORL通过高效的分解与优化机制，为复杂多目标决策问题提供了一种更优的帕累托前沿近似方案。
---
