...
tldr: 针对深度强化学习（DRL）在工业批处理控制中存在的随机不确定性及缺乏稳定性保证等问题，本文提出了一种迭代学习控制引导的强化学习（IL-CIRL）框架。该框架结合了ILC的重复性优化优势与DRL的自适应能力，通过卡尔曼滤波状态估计和双层控制架构，引导智能体学习满足约束且具备稳定性保证的策略，有效提升了批处理过程在复杂扰动下的控制性能。
selection_source: fresh_fetch
motivation: 深度强化学习在处理工业批处理过程时，因动作随机性带来的安全风险和缺乏稳定性保证限制了其在实际工业中的应用。
method: 提出IL-CIRL框架，在双层控制架构中利用基于卡尔曼滤波的迭代学习控制结构来指导DRL智能体的策略训练。
result: 该方法使DRL控制器能够在满足操作约束的同时确保系统稳定性，并能系统地应对批处理过程中的多种扰动条件。
conclusion: IL-CIRL成功将传统控制理论的稳定性保障与强化学习的灵活性相结合，为批处理过程提供了一种系统化且安全的控制器设计方案。
---
