...
motivation: 旨在解决强化学习在动力学环境变化时性能难以保证且其内部优化机制缺乏直观解释手段的问题。
method: 通过将Critic参数轨迹投影至低维线性子空间，并结合固定参考样本与时序差分目标构建三维损失景观及定量评价指标。
result: 在倒立摆和航天器姿态控制任务中，该方法成功揭示了稳定收敛与不稳定学习过程中截然不同的损失景观特征。
conclusion: 该可视化框架为在线强化学习中Critic的优化行为提供了有效的定性与定量解释手段，有助于深入理解算法机制。
---
